...
// Double-load kernel: each thread loads and adds 2 elements before reduction
__global__ void sumReductionIdleThreads(int *v, int *v_r) {
    __shared__ int partial_sum[SHMEM_SIZE];

    // Each block covers blockDim.x * 2 elements
    int i = blockIdx.x * (blockDim.x * 2) + threadIdx.x;
    partial_sum[threadIdx.x] = v[i] + v[i + blockDim.x];
    __syncthreads();

    // Standard fold-in reduction (same as no_conflicts variant)
...
int main(int argc, char *argv[]) {
    ...
    sumReductionIdleThreads<<<GRID_SIZE, TB_SIZE>>>(d_v, d_v_r);
    {
        ...
        // Multi-stage: each stage halves via double-load
        while (remaining > 1) {
            if (remaining >= TB_SIZE * 2) {
                int next_grid = remaining / (TB_SIZE * 2);
                sumReductionIdleThreads<<<next_grid, TB_SIZE>>>(src, dst);
                remaining = next_grid;
                int *tmp = src; src = dst; dst = tmp;
            } else {
                // Final stage: remaining fits in one block with remaining/2 threads
                int final_threads = remaining / 2;
                if (final_threads < 1) final_threads = 1;
                sumReductionIdleThreads<<<1, final_threads>>>(src, src);
                remaining = 1;
            }
        }
        int gpu_result;
