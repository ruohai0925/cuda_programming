...
// Sequential indexing: eliminates divergence but introduces bank conflicts
__global__ void sumReduction(int *v, int *v_r) {
    ...
    // Sequential mapping: index = 2 * s * threadIdx.x
    // Consecutive threads work on consecutive indices (no divergence)
    // But stride of 2*s causes shared memory bank conflicts
